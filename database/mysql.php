...
class mysql {
	...
	public function getList($sql){
		...
		if($sql == ''){
			$arr['code']='1001';
			$arr['msg'] = 'SQL is Empty';
			return $arr;
		}
		$rs = mysql_query($sql, $this->conn);
		while($row = mysql_fetch_assoc($rs)){
			array_push($arr['list'], $row);
		}
		$arr['code'] = '200';
		$arr['msg'] = 'Success';
		return $arr;
	...
	*/
	...
	public function delete($table, $condition, $url = '') {
		if ($this->query("DELETE FROM $table WHERE $condition")) {
			if (!empty($url)) $this->Get_admin_msg($url, '删除成功！');
		}
	}
	//简化插入insert
	public function insert($table, $columnName, $value, $url = '') {
		if ($this->query("INSERT INTO $table ($columnName) VALUES ($value)")) {
			if (!empty($url)) $this->Get_admin_msg($url, '添加成功！');
		}
	}
	...
	public function update($table, $mod_content, $condition, $url = '') {
		...
		if ($this->query("UPDATE $table SET $mod_content WHERE $condition")) {
			if (!empty($url)) $this->Get_admin_msg($url);
		}
	}
	...
	function checkurl() { //检查来路
		if (preg_replace("/https?:\/\/([^\:\/]+).*/i", "\\1", $_SERVER['HTTP_REFERER']) !== preg_replace("/([^\:]+).*/", "\\1", $_SERVER['HTTP_HOST'])) {
			header("Location: http://www.janff.com");
			exit();
		}
	}
}
...
//$db = new mysql($DB_host, $DB_user, $DB_password, $DB_database, '', 'utf8');
?>
